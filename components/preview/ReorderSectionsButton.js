/**
 * ReorderSectionsButton Component
 * 
 * Button that opens the SectionOrderManager modal.
 * Can be used standalone or within a dropdown menu.
 */
...
export default function ReorderSectionsButton({ onClose, className = "" }) {
  ...
  // Use SectionOrderManager's logic but render our own button
  const sectionLabels = {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-dark-700 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-dark-500 dark:text-light-400 mb-4">
              Drag and drop sections to reorder them, or use the arrow buttons below.
            </p>

            <div className="space-y-2 mb-4">
              {state.sectionOrder.map((section, index) => (
                <React.Fragment key={section}>
                  {dropTarget === section && draggedItem && draggedItem !== section && (
                    <div
            ...
            <div className="flex gap-x-2">
              ...
              <button onClick={handleClose} className="btn-brand btn-sm flex-1">
                Done
              </button>
            </div>
          </div>
