...
export function stateToProfileJson(state) {
  ...
  // Merge with default profile so we always have full schema (badges, etc.) and save never fails
  const defaultProfile = getDefaultProfileJson().profile;
  const stateProfile = {
  ...
  const mergedProfile = {
    ...defaultProfile,
    ...stateProfile,
    introduction: { ...defaultProfile.introduction, ...stateProfile.introduction },
    badges: { ...defaultProfile.badges, ...(stateProfile.badges || {}) },
    support: { ...defaultProfile.support, ...(stateProfile.support || {}) },
  };
...
export function profileJsonToState(profileJson, existingState = {}) {
  ...
  return {
    ...
    mainTab: existingState.mainTab ?? "profile",
